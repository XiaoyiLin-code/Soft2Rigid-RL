...
import json
# Step 1: 加载 URDF 和创建链
urdf_path = "/home/lightcone/workspace/SOFA/mlp/finger/urdf/finger.urdf"
...
class FingerMLP(nn.Module):
    def __init__(self,  input_dim=16, output_dim=16,start=0):
        ...
        self.lower_joint_limits = torch.tensor([0]*6).to("cuda:0")
        self.upper_joint_limits = torch.tensor([math.pi/4]*6).to("cuda:0")
...
# 加载 JSON 数据
json_path = "./displacement_position.json"  # 替换为你的 JSON 文件路径
inputs, outputs = load_data_from_json(json_path)

# 插值补充数据
interp_inputs, interp_outputs = interpolate_data_and_save(inputs, outputs, num_samples=300)

# 将原始数据与插值数据合并
inputs.extend(interp_inputs)
outputs.extend(interp_outputs)
keypoint_names = [
    "keypoint_0_virtual", "keypoint_1_virtual", "keypoint_2_virtual", 
    "keypoint_3_virtual", "keypoint_4_virtual", "keypoint_5_virtual"
]
...
outputs[:,:,1]+=10
def random_sample_data(inputs, outputs, sample_size):
    """
    从输入和输出中随机采样并返回 PyTorch 张量。
    Args:
        inputs: 输入数据（列表或 NumPy 数组）。
        outputs: 输出数据（列表或 NumPy 数组）。
        sample_size: 要采样的数量。
    Returns:
        sampled_inputs_tensor: 采样的输入张量。
        sampled_outputs_tensor: 采样的输出张量。
    """
    # 获取总样本数
    ...
    # 确保结构一致
    sampled_inputs_array = np.array(sampled_inputs, dtype=np.float32)
    sampled_outputs_array = np.array(sampled_outputs, dtype=np.float32)

    # 转换为 PyTorch 张量
    sampled_inputs_tensor = torch.tensor(sampled_inputs_array, dtype=torch.float32)
    sampled_outputs_tensor = torch.tensor(sampled_outputs_array, dtype=torch.float32)

    return sampled_inputs_tensor, sampled_outputs_tensor


sample_size = 100000
s, outputs = random_sample_data(s, outputs, sample_size)
...
# 创建数据加载器
print(s.shape,keypoints_target.shape)
...
print("预测关节角度:", q_test)
print("预测关键点位置:", keypoints_pred,keypoints_true,keypoints_true-keypoints_pred)
# 保存模型权重

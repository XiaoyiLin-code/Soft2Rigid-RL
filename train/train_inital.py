...
# Step 1: 加载 URDF 和创建链
urdf_path = "/home/lightcone/workspace/SOFA/左手/finger_new/urdf/finger_new.urdf"
...
class FingerMLP(nn.Module):
    def __init__(self,  input_dim=16, output_dim=16,start=0):
        ...
        self.upper_joint_limits = torch.tensor([math.pi]*6).to("cuda:0")
...
# 输入：随机生成绳长数据 s
# 输入：随机生成绳子拉力
s = torch.rand(N, M, dtype=torch.float32)  # 绳子拉力 (N, M)

# 比例系数 k，用于将绳子拉力转换为关节角度
k = 2.0  # 可以根据实际需求调整比例系数

# 输出：生成关节角度，所有关节值相同
# q_targets 的形状为 (N, joint_dim)，每个样本的所有关节值相同
q_targets = (k * s).repeat(1, joint_dim)  # 拉力值按比例生成关节角度，并复制到所有关节
# 输出：通过 pytorch_kinematics 计算目标关键点位置
keypoint_names = ["keypoint_0_virtual","keypoint_1_virtual","keypoint_2_virtual","keypoint_3_virtual","keypoint_4_virtual","keypoint_5_virtual"]  # 假设两个关键点
keypoints_target = forward_kinematics(chain, q_targets, keypoint_names)

print(keypoints_target,keypoints_target.shape)
# Step 5: 定义损失函数
...
q_ture = (k * s_test).repeat(1, joint_dim) 
keypoints_pred = forward_kinematics(chain, q_test, keypoint_names)  # 预测关键点位置
keypoints_true = forward_kinematics(chain, q_ture, keypoint_names)  # 预测关键点位置
